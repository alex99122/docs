...
<ParamField path="type" type="string" required>
  ...
  - `pending_review`: Payment flagged for fraud review. Do NOT fulfill until you receive `succeeded` or `declined`.
  ...
  - `incomplete`: A crypto payment came in short (the customer sent less than the requested amount). Opt-in event — enable it in your webhook settings. Includes `amount_remaining` (how much is still due). Do NOT fulfill until you receive `succeeded`.
</ParamField>
...
<ParamField path="amount_remaining" type="integer">
  Only present on `incomplete` events. The amount still due, in the smallest currency unit (e.g., cents for USD), i.e. how much more the customer must send to complete the payment. The amount already received equals `amount - amount_remaining`.
</ParamField>
...
public function handleWebhook(Request $request) {
    ...
    switch ($data['type']) {
        ...
        case 'expired':
            $this->handleExpiredSession($data['order_id']);
            break;
    }
...
app.post('/api/webhook', express.raw({ type: 'application/json' }), (req, res) => {
    ...
    switch (data.type) {
        ...
        case 'expired':
            handleExpiredSession(data.order_id);
            break;
    }
...
def handle_webhook():
    ...
    elif data['type'] == 'expired':
        handle_expired_session(data['order_id'])
...
func handleWebhook(w http.ResponseWriter, r *http.Request) {
    ...
    case "expired":
        handleExpiredSession(data.OrderID)
    }
