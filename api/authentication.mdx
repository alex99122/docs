...
All API requests must be authenticated using your API key. You can find your API keys in the [Payviox Dashboard](https://dashboard.payviox.com) under the Developer section.
...
```bash Public API Key (Client-side)
Authorization: Bearer pk_live_xxxxxxxxxxxx
```

```bash Secret API Key (Server-side)
