...
<Tip>
The `type` field indicates the payment event type: `succeeded`, `pending_review`, `declined`, `refunded`, `expired`. The `fees` field indicates total fees in the smallest currency unit - customer pays `amount + fees = total`.
</Tip>
